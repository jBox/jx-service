...
const validateRegister = (req, res, next) => {
    ...
    if (!isUsernameValid(username)) {
        error.message = "用户名不能为空，必须以字母开头，只能包含字母和数字或者(-,_)，长度不能小于5。";
    }
    ...
    req.register = { nickname, username, password };
    return next();
}

const register = (req, res, next) => {
    const { userService } = req.services;
    const { nickname, username, password } = req.register;

    return userService.register(nickname, username, password).then((reg) =>
        res.send(reg)
