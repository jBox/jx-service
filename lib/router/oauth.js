const { validateMobileNumber, isCaptchaValid, isUsernameValid } = require("../utils");
...
router.post("/authorize", (req, res, next) => {
    ...
    if (type === "authorization_code" && oauthService.secret(secret)) {
        ...
        const { userService, captchaService } = req.services;
        const { mobile, captcha, username, password } = req.body;
        let validator = null;
        if (validateMobileNumber(mobile) && isCaptchaValid(captcha)) {
            validator = userService.verifyMobile(mobile).then((user) => {
                return captchaService.verify("login", mobile, captcha).then((verified) => {
                    if (verified) {
                        return user;
                    }
