...
const update = [
    ...
    (req, res, next) => {
        ...
        const { orderService, imageService } = req.services;

        if (operation === "progress") {
            return imageService.pics(data).then((progress) => {
                return orderService[operation](user, id, Number(version), null, progress).then((order) => {
