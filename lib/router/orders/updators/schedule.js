...
module.exports = (req, res, next) => {
    ...
    return orderService.get(id).then((item) => {
        ...
        return orderService.schedule(user, id, Number(version), schedules).then((order) => {
            ...
            smsTasks.push(smsService.orderScheduled(order.mobile, order.contact, { orderId: order.id })
                .catch((error) => {
            ...
            for (let schedule of newschedules) {
                ...
                smsTasks.push(smsService.driverScheduled(schedule.mobile, schedule.driver, smsText)
                    .catch((error) => {
                        warning.message = "短信通知失败：" + error;
                        console.error("Scheduled text to driver", error);
                    }));
            }
