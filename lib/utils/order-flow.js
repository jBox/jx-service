...
class OrderFlow {
    ...
    schedule(user, schedules/*[{id,licenseNumber, model, driver, mobile}]*/) {
        ...
        for (let schedule of schedules) {
            const key = `${schedule.mobile}${this.order.id}`;
            updates[key] = { ...schedule, orderId: this.order.id };
        }
        ...
        const traces = [
            ...this.order.traces,
            {
                ...
                state: schedules.map((schedule) => (
                    `出车安排: ${driver}, ${mobile}, ${licenseNumber}`
                )).join(" / "),
                time: now()
            }
        ];
    ...
    depart(user) {
        ...
        const driverSchedule = this.order.schedules.find(x => x.mobile === user.mobile);
        if (!driverSchedule) {
            throw new Error(ILLEGAL_OPERATION);
        }
    ...
    progress(user, data) {
        ...
        const driverSchedule = this.order.schedules.find(x => x.mobile === user.mobile);
        if (!driverSchedule || !driverSchedule.status) {
            throw new Error(ILLEGAL_OPERATION);
        }
    ...
    complete(user) {
        ...
        const allSchedulesEnded = this.order.schedules.reduce((ended, s) => (ended && s.status === "end"), true);
