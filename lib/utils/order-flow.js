...
class OrderFlow {
    ...
    cancel(customer) {
        ...
        if (this.order.schedules.some(x => !!x.status)) {
            throw new Error(ILLEGAL_OPERATION);
        }
    ...
    schedule(user, schedules/*[{belongs,licenseNumber, model, driver, mobile}]*/) {
        if (this.order.service.status === ORDER_STATUS.cancelled.id) {
            throw new Error(ILLEGAL_OPERATION);
        }
        ...
        const traces = [
            ...this.order.traces,
            {
                ...
                state: `出车安排: ${schedules.map((schedule) => (
                    `${schedule.licenseNumber}, ${schedule.driver}（${schedule.mobile}）`
                )).join(" / ")}`,
                time: now()
            }
        ];
    ...
    progress(user, schedule, data) {
        ...
        if (this.orderStatus.le(ORDER_STATUS.scheduled.id)) {
            throw new Error(ILLEGAL_OPERATION);
        }
