...
class OrderService {
    ...
    schedule(user, id, version, data) {
        ...
        const schedules = data.reduce((items, item) => {
            if (item.id && item.licenseNumber && item.mobile) {
                return items.concat({
                    id: item.id,
        ...
        return this.get(id).then((order) => {
            ...
            for (let key of Object.keys(scheduled.updates)) {
                ...
                if (value === null) {
                    ...
                    scheduleUpdates.push({ type: "put", key, value });
                }
            }
    ...
    schedules() {
        return new Promise((resolve, reject) => {
            ...
            db.schedules.createReadStream()
                .on("data", (data) => {
                    results[data.key] = JSON.parse(data.value);
                })
                .on("error", (err) => reject(err))
    ...
    revert(user, id, version) {
        return this.get(id).then((order) => {
            ...
            const flow = new OrderFlow(order);
            const revertedOrder = flow.revert(user);
            const data = JSON.stringify({ ...revertedOrder, version: Date.now() });
            const tasks = [db.instance.put(revertedOrder.id, data).then(() => JSON.parse(data))];
            if (scheduleUpdates.length > 0) {
                tasks.push(db.schedules.del(`${user.mobile}${revertedOrder.id}`));
            }
    ...
    progress(user, id, version, progress) {
        return this.get(id).then((order) => {
            if (order.version !== version) {
                return Promise.reject(new BadRequestError());
            }
            ...
            const flow = new OrderFlow(order);
            return this.put(flow.progress(user, progress));
        });
    }
