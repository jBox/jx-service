...
class OrderService {
    ...
    cancel(user, id, version, exec) {
        return this.get(id).then((order) => {
            ...
            if (exec) {
                const updatedOrder = flow.confirmCancel(user);
                const scheduleUpdates = updatedOrder.schedules.map((item) => (
                    { type: "del", key: `${item.mobile}${updatedOrder.id}` }
                ));

                const data = JSON.stringify({ ...updatedOrder, version: Date.now() });
                const tasks = [db.instance.put(updatedOrder.id, data).then(() => JSON.parse(data))];
                ...
                return this.put(flow.cancel(user));
            }
        });
    }
    ...
    confirm(user, id, version, department) {
        return this.get(id).then((order) => {
            if (order.version !== version) {
                return Promise.reject(new BadRequestError());
            }
            ...
            const flow = new OrderFlow(order);
            const updated = flow.confirm(user);
            if (department) {
                updated.department = department.trim();
            }
