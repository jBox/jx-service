...
const { ORDER_STATUS, ORDER_EDITABLE_FIELDS } = require("../utils/constants");
...
class OrderService {
    constructor(dataDir) {
        ...
        if (!db.instance) {
            ...
            indicateIdentity(db.instance).then((id) => db.MAX_ID = id);
        }
    }
    ...
    cancel(id, version, exec) {
        return this.get(id).then((order) => {
            if (order.version !== version) {
                return Promise.reject(new BadRequestError());
            }

            return this.put(exec ? flow.exeCancel(order) : flow.cancel(order));
        });
    }
