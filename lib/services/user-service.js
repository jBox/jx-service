...
class UserService {
    ...
    verifyMobile(mobile) {
        return db.mobile.get(mobile).then(data => JSON.parse(data)).then((mob) => ({ id: mob.id }));
    }
    ...
    create(nickname, mobile, roles, options) {
        if (!this.rolesService.available(roles)) {
            return Promise.reject("角色不存在，请联系管理员！");
        }
        ...
        const pre = [
            db.nextIdentity(),
            db.register.get(mobile).then(() => Promise.resolve(false)).catch(() => Promise.resolve(true)),
            db.mobile.get(mobile).then(() => Promise.resolve(false)).catch(() => Promise.resolve(true))
        ];

        return Promise.all(pre).then(([id, existsReg, existsMob]) => {
            if (!reg) {
                return Promise.reject("手机号码不合法，请联系管理员！");
            }
