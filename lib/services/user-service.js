...
class UserService {
    ...
    lookup(identity) {
        const tasks = [
            this.get(identity),
            db.mobile.get(identity).then((data) => JSON.parse(data)).catch((error) => {
                if (error.notFound) {
                    return Promise.resolve();
                }
            ...
            db.secure.get(identity).then((data) => JSON.parse(data)).catch((error) => {
                if (error.notFound) {
                    return Promise.resolve();
                }
        ...
        return Promise.all(tasks).then(([user, mob, sec]) => {
            if (user) {
                return user;
            }

            if (mob) {
                return this.get(mob.id);
            }
